module.exports.config = {
  ...
  version: "1.0.0",
  credits: "Mirai-Team", // FIXED BY SM Siyam
...
let gifPath = __dirname + '/Siyam/join/join.gif';
...
DEVELOPER :Mohammad Siyam

🟣Facebook Account Link: 

https://www.facebook.com/Siyam6252?mibextid=ZbWKwL

🔵WHATSAPP NUMBER: wa.me/+8801533969430

🟢SUPPORT EMAIL: www.mdsiamhasan6252@gmail.com`, attachment: fs.createReadStream(gifPath)}, threadID));
  }})
.catch(error => {
    console.error(error);
});
    ...
    try {
      if (!fs.existsSync(__dirname + `/Siyam/font/Semi.ttf`)) {
        let getfont = (await axios.get(fontlink, { responseType: "arraybuffer" })).data;
        fs.writeFileSync(__dirname + `/Siyam/font/Semi.ttf`, Buffer.from(getfont, "utf-8"));
      };
      ...
      for (let o = 0; o < event.logMessageData.addedParticipants.length; o++) {
        let pathImg = __dirname + `/Siyam/join/${o}.png`;
        let pathAva = __dirname + `/Siyam/join/avt.png`;
        ...
        registerFont(__dirname + `/Siyam/font/Semi.ttf`, {
          family: "Semi"
        });
        ...
        abx.push(fs.createReadStream(__dirname + `/Siyam/join/${o}.png`))
      }
      ...
      for (let ii = 0; ii < parseInt(id.length); ii++) {
        fs.unlinkSync(__dirname + `/Siyam/join/${ii}.png`)
      }
